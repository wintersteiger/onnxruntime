...
Status Concat::Compute(OpKernelContext* ctx) const {
  ...
  for (int input_index = 0; input_index < input_count; input_index++) {
    ...
    int64_t in_offset = 0;
    for (size_t idxCopy = 0, end = input_size / input_axis_pitch; idxCopy < end; ++idxCopy) {
      if (is_string_type) {
        size_t out = initial_output_offset + out_offset;
        for (int idxItem = 0; idxItem < input_axis_pitch; ++idxItem) {
          reinterpret_cast<std::string*>(output)[out + idxItem] =
              reinterpret_cast<const std::string*>(input)[in_offset + idxItem];
        }
      } else {
        memcpy(
            output + (initial_output_offset + out_offset) * element_bytes,
            input + in_offset * element_bytes,
            input_axis_pitch * element_bytes);
      }

      out_offset += p.output_axis_pitch;
      in_offset += input_axis_pitch;
    }
