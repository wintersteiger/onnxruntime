...
InferenceSession::InferenceSession(const SessionOptions& session_options, logging::LoggingManager* logging_manager)
  ...
  ORT_ENFORCE(Environment::IsInitialized(), "Environment must be initialized before creating an InferenceSession.");
...
common::Status InferenceSession::CreateSubgraphSessionState(Graph& graph, SessionState& session_state) {
  for (auto& node : graph.Nodes()) {
    for (auto& entry : node.GetAttributeNameToMutableSubgraphMap()) {
      ...
      auto subgraph_session_state = std::make_unique<SessionState>(
          execution_providers_, session_state.GetEnableMemoryPattern(), session_state.GetThreadPool());
...
common::Status InferenceSession::Initialize() {
  ...
  try {
    ...
    if (!execution_providers_.Get(onnxruntime::kCpuExecutionProvider)) {
      ...
      ORT_RETURN_IF_ERROR(
          execution_providers_.Add(onnxruntime::kCpuExecutionProvider, std::make_unique<CPUExecutionProvider>(epi)));
    }

    // Register data transfer methods.
    data_transfer_mgr_.RegisterDataTransfer(std::make_unique<CPUDataTransfer>());
#ifdef USE_CUDA
    // TODO: this should be refactored later by exposing separate API to allow users to register different data
    // transfers for different devices.
    bool is_nvidia_gpu_used = (nullptr != execution_providers_.Get(kCudaExecutionProvider)) ||
                              (nullptr != execution_providers_.Get(kTensorrtExecutionProvider));
